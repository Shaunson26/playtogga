...
playerMLStats = data.frame(POS = playerURLTeamPosition$POS[sapply(pNnames, function(x) which(x == playerURLTeamPosition$Name))],
                           playerMLStats)

(predictedPoints = calculateBytoggaScores(list(playerMLStats), eventScores))[1:10,]

stripchart(predictedPoints, method = "jitter", vert = T, ylab = "Predicted points", font.lab = 2,
           pch = 19)
```
...
```{r}
(predictedPointsQQ = calculateBytoggaScores(qqScoresList, eventScores))[1:10,]
boxplot(predictedPointsQQ)
```
